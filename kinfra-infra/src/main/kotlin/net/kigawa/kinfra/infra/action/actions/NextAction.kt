...
class NextAction(
    ...
    override fun execute(args: List<String>): Int {
        ...
        try {
            ...
            if (hasLocalChanges) {
                ...
                // Stash local changes
                val stashResult = processExecutor.execute(
                    arrayOf("git", "stash", "push", "-m", "kinfra-next-auto-stash"), repoPath
                )
                when (stashResult) {
                    is Res.Err -> {
                ...
                is Res.Err -> {
                    ...
                    if (stashed) {
                        ...
                        when (popResult) {
                            is Res.Err -> {
                                println(
                                    "${AnsiColors.YELLOW}⚠ Failed to restore stashed changes due to conflicts${AnsiColors.RESET}"
                                )
                                println(
                                    "${AnsiColors.BLUE}The stash entry is kept in case you need it again.${AnsiColors.RESET}"
                                )
                                println(
    ...
    private fun enableUpdatedSubProjects(repoPath: File): Int {
        try {
            ...
            println(
                "${AnsiColors.GREEN}✓ Enabled ${changedSubProjects.size} changed sub-project(s):${AnsiColors.RESET}"
            )
