...
import world.onemc.domain.OneLogger
            ...
            it.build(contextBundle.get<CoroutineContext>().asScope().async {
                supervisorScope {
                    try {
                        ...
                        val logger = contextBundle.getOrNull(OneLogger::class) ?: OneLogger.Default
                        logger.error("scope error", e)
                        logger.error("scope block $block")
